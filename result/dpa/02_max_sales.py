...
def main():
    ...
    for i in range(len(testcases_1)):
        ...
        for unit, order in sales_per_unit.items():
            print(f"     {unit}세트 의뢰 : {order}회")
        print()
    print("최소 판매 세트 제한이 있는 경우 (보너스 과제)")
    for i in range(len(testcases_2)):
        print(f"TC {i + 1}: ")
        max_price, memo_sales_unit = max_sales_limit(len(testcases_2[i]), testcases_2[i])
        print(f"  최대 매출 : {max_price}억원")
        sales_way, remainer_unit = sales_unit_limit(len(testcases_2[i]), memo_sales_unit)
        sales_per_unit = {}
        for unit in sales_way:
            if unit not in sales_per_unit:
                sales_per_unit[unit] = 1
        ...
        for unit, order in sales_per_unit.items():
            print(f"     {unit}세트 의뢰 : {order}회")
        print(f"  잔여 처리 용량 : {remainer_unit}세트")
        print()
    # testcase_1 = 
    # prices = []
