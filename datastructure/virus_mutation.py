...
def random_virus_name(length):
    result = ''
    for _ in range(length):
        result += chr(random.randint(97, 122))
    return result

def make_random_graph(edge_count, vertex_count):
    virus_list = []

    while True:
        if len(virus_list) == vertex_count:
            break
        name = random_virus_name(5)
        if name not in virus_list:
            virus_list.append(name)
    #virus_list[0] = 'start'
    vg = VirusMutations()
    for _ in range(edge_count):
        before = random.choice(virus_list)
        after = random.choice(virus_list)
        vg.add_process(before, after)
    return vg
